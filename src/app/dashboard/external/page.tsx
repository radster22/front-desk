...
const updateRequestStatuses = (data: RequestType[]) => {
    const now = new Date();
    return data.map(request => {
      const createdAtDate = new Date(request.createdAt);
      const hoursDifference = (now.getTime() - createdAtDate.getTime()) / (1000 * 60 * 60);

      // If request is "new" and older than 24 hours, mark as "open"
      if (request.status.toLowerCase() === "new" && hoursDifference >= 24) {
  ...
  useEffect(() => {
    const fetchRequests = async () => {
      try {
        ...
        // Update statuses before setting state
        const updatedData = updateRequestStatuses(data);
  ...
  return (
    <div className="flex">
      ...
      <div className="flex-1 p-8 bg-gray-100">
        <h1 className="text-2xl font-bold mb-6 text-blue-900">My Requests</h1>
              ...
              <table className="min-w-full bg-white border-collapse border rounded-md">
                ...
                <tbody>
                  {requests.map(request => (
                    <tr 
                      key={request.id} 
                      onClick={() => router.push(`/dashboard/external/${request.id}`)}
